          ...
          `
                ...
                commentLike.addEventListener('click', function () {
                    ...
                    if (commentLike.style.color === 'red') {
                        ...
                        commentLike.firstElementChild.setAttribute('src', "/images/heart2.png")
                        ...
                        commentLike.firstElementChild.setAttribute('src', "/images/heart1.png")
...
likesBtn.forEach(like => {
    ...
    like.addEventListener('click', function () {
        ...
        let youParagraphBlade = document.querySelector(
            `#you-like-blade-${postId}`
        )
        if (youParagraphBlade) youParagraphBlade.style.display = 'none'

        if (like.style.color === 'red') {
            like.style.color = 'black'
            let imgDef = like.getAttribute('data-img-src-default')
            like.firstElementChild.setAttribute('src', imgDef)
            axios.delete(`/like/destroy/${likeId}`).then(res => {
                let likes_count = document.querySelector(
                    `.likes-count-${postId}`
                )
                ...
                if (res.data.user.length > 0) {
                    ...
                    let othersContent = document.querySelector(
                        `.othersContent-${postId}`
                    )
            ...
            let imgDef = like.getAttribute('data-img-src')
            like.firstElementChild.setAttribute('src', imgDef)
            axios.post('/like', { id: postId }).then(res => {
                ...
                let others_btn = document.querySelector(`#others-btn-${postId}`)
...
bookMarkBtn.forEach(bookMark => {
    bookMark.addEventListener('click', function () {
        ...
        if (bookMark.style.color === 'orange') {
            ...
            let imgDef = bookMark.getAttribute('data-img-src-default')
            bookMark.firstElementChild.setAttribute('src', imgDef)
            axios.delete(`/save/destroy/${postId}`).then(res => {
                // console.log(res.data);
            })
        } else {
            bookMark.style.color = 'orange'
            let imgDef = bookMark.getAttribute('data-img-src')
            bookMark.firstElementChild.setAttribute('src', imgDef)
...
commentButton.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        axios.get(`/post/${postId}`).then(res => {
                    ...
                    <div class="container-fluid">
                        <div class="row p-0">
                            <div class="col-7 col-lg-7 col-md-12 col-sm-12 d-flex p-0 m-0">
                                <img src="${res.data.post.image_url
                ...
                res.data.post.user.id
                    ...
                    : `<div class="comments-modal-container">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="d-flex">
                                            <p class="m-1 mx-3" id="likes_modal_count-${postId}">${res.data.all_likes_count
                } Likes</p>
                                        ...
                                        <div class="px-3">
                                            <p class=" text-secondary"><i>${res.data.posts_time
                }</i></p>
                                        </div>
                                    </div>
                                    <!---------------------------------------------------------->
            ...
            let postLike = document.getElementById(
                `postLike-${res.data.post.id}`
            )
            ...
            if (res.data.post_like.likes.length == 0) {
                let imgDef = postLike.getAttribute('data-img-src-default')
                if(postLikeModal) postLikeModal.firstElementChild.setAttribute('src', imgDef)
            }
            res.data.post_like.likes.map(function (x) {
                postLikeModal.setAttribute('data-bs-like', `${x.id}`)

                if (res.data.logged_user == x.user_id) {
                    let imgDef = postLike.getAttribute('data-img-src')
                    if(postLikeModal) postLikeModal.firstElementChild.setAttribute('src', imgDef)
                    console.log(imgDef)
            ...
            postLikeModal.addEventListener('click', function () {
                ...
                if (postLike.style.color === 'red') {
                    ...
                    let imgDef = postLike.getAttribute('data-img-src-default')
                    if(postLike) postLike.firstElementChild.setAttribute('src', imgDef)
                    if(postLikeModal) postLikeModal.firstElementChild.setAttribute('src', imgDef)

                    axios.delete(`/like/destroy/${likeId}`).then(res => {
                        // console.log(res.data);

                        let likes_modal_count = document.querySelector(
                            `#likes_modal_count-${postId}`
                        )
                        ...
                        let youLike = document.querySelector(
                            `#you-like-${postId}`
                        )
                        if (res.data.user.length > 0) {
                            if (youLike) {
                    ...
                    axios.post('/like', { id: postId }).then(res => {
                        ...
                        let commentLikesDiv = document.querySelector(
                            `#likes-${postId}`
                        )
                        let commentDetails = `
                    <div class="likess othersContent-${postId}">
            ...
            if (res.data.saved_posts.savedposts.length == 0) {
                let imgDef = postBookMark.getAttribute('data-img-src-default')
                if(postBookMark) postSaveModal.firstElementChild.setAttribute('src', imgDef)
            }
            ...
            res.data.saved_posts.savedposts.map(function (x) {
                ...
                if (res.data.logged_user == x.pivot.user_id) {
                    let imgDef = postBookMark.getAttribute('data-img-src')
                    if(postBookMark) postSaveModal.firstElementChild.setAttribute('src', imgDef)
                    ...
                    let imgDef = postBookMark.getAttribute(
            ...
            postSaveModal.addEventListener('click', function () {
                ...
                if (postBookMark.style.color === 'orange') {
                    ...
                    if(postBookMark)  postBookMark.firstElementChild.setAttribute('src', imgDef)
                    if(postSaveModal) postSaveModal.firstElementChild.setAttribute('src', imgDef)
            ...
            if (ConfirmMessage) {
                ConfirmMessage.onclick = function () {
                    ...
                    axios
                        .delete(`/comment/destroy/${commentId}`)
                        .then(response => {
                            let commentDiv = document.querySelector(
                                `.comment-div-${commentId}`
                            )
                            if (commentDiv) {
                                commentDiv.remove()
                            }
                            ...
                            let commentInBlade = document.querySelector(
                                `.user-comment-${commentId}`
                            )
                            if (commentInBlade) {
                            ...
                            let noComment = document.getElementById(
                                `No-Comment-${res.data.post.id}`
                            )
                            ...
                            let modalNoComments = document.querySelector(
                                '.modal-all-comments-display'
                            )
                            ...
                            res.data.allComments.length =
                                res.data.allComments.length - 1
                            ...
                            let viewAllComments = document.getElementById(
                                `view-all-comments-${res.data.post.id}`
                            )
                            if (viewAllComments) {
                                let commentsCount = parseInt(
                                    viewAllComments.innerText.split(' ')[2]
                                )
                                if (commentsCount > 0) {
                                    viewAllComments.innerText = `View all ${commentsCount - 1
                                        } comments`
                                }
                            }
                        })
                ...
                document
                    .querySelectorAll('#deleteConfirmationModal .close')
                    .forEach(btn => {
            ...
            res.data.post_comment_id.comments.forEach(comment => {
                let postCommentLike = document.getElementById(
                ...
                if (postCommentLike && res.data.logged_user == comment.user_id) {
                    ...
                    if (postCommentLike && postCommentLike.style.color === 'red') {
                        ...
                        postCommentLikeModalClass.firstElementChild.setAttribute("src", "/images/heart1.png");
                        postCommentLikeModalClass.setAttribute(
                            'style',
                            'color: red !important'
                        )
                    } else {
                        postCommentLikeModalClass.firstElementChild.setAttribute("src", "/images/heart2.png");
                        postCommentLikeModalClass.setAttribute(
                            'style',
                    ...
                    postCommentLikeModalClass.addEventListener('click', function () {
                        ...
                        let commentLikeId = postCommentLike.getAttribute(
                            'data-bs-commentLike'
                        )
                        ...
                        if (postCommentLikeModalClass.style.color === 'red') {
                            ...
                            let imgDef = "/images/heart2.png"
                            if(postCommentLike) postCommentLike.firstElementChild.setAttribute("src",imgDef);
                            if(postCommentLikeModalClass) postCommentLikeModalClass.firstElementChild.setAttribute("src",imgDef);
                    ...
                    let comment_likes_id = document.querySelector(
                        `.comment-like-btn-${comment.id}`
                    )
                    ...
                    for (let i = 0; i < allLikes.length; i++) {
                        if (
                            comment.id == allLikes[i].comment_id &&
                            postId == allLikes[i].post_id &&
                            res.data.logged_user == allLikes[i].user_id
                        ) {

                            comment_likes_id.firstElementChild.setAttribute("src","/images/heart1.png");
                            comment_likes_id.setAttribute('style', 'color: red')
                            ...
                            comment_likes_id.firstElementChild.setAttribute("src","/images/heart2.png");
                            comment_likes_id.setAttribute('style', 'color: grey')
                        }
                    }

                    postCommentLikeModalClass.addEventListener('click', function () {
                        let postId =
                            postCommentLikeModalClass.getAttribute('data-bs-postId')
                        let commentId = postCommentLikeModalClass.getAttribute(
                            'data-bs-postCommment'
                        )
                        let commentLikeId = postCommentLikeModalClass.getAttribute(
                            'data-bs-commentLike'
                        )

                        let postCommentLike = document.querySelector(
...
function commentLikesModalDiv(commentsLikesUsersArr) {
    ...
    commentsLikesUsersArr.forEach(like => {
        ...
        if (commentDiv) {
            let likesContainer = commentDiv.querySelector(
                `.allCommentLikesUsers-${commentId}`
            )
            if (!likesContainer) {
                likesContainer = document.createElement('div')
                likesContainer.classList.add(
                    `allCommentLikesUsers-${commentId}`
                )
                ...
                likesContainer.innerHTML += `
                        <div class="row w-100 d-flex align-items-center justify-content-between m-0 mb-2">
                            <div class="col-9 d-flex align-items-center">
                                ...
                                class="rounded-circle avatar" width="40px"
                                height="40px" alt="Avatar"/>
                                <a
                                href="${'/profile/' + user.user.id}"
                                type="button" class="w-100 mx-3" >
                                    <b>${userName}</b>
                                </a>
                            </div>
                        </div>
...
followBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        console.log(btn)
        let userSuggestId = document.getElementById(
            `follow-btn-suggest-${userId}`
        )
        ...
        if (followType == 'follow') {
            axios.post('/follow', { user_id: userId }).then(res => {
                ...
                if (userSuggestId) {
                    ...
                    if (userSuggestId.classList.contains('btn-primary'))
                        userSuggestId.classList.remove('btn-primary')
                ...
                if (postUserId) {
                    ...
                    if (postUserId.classList.contains('btn-primary'))
                        postUserId.classList.remove('btn-primary')
            ...
            axios.delete(`/following/${userId}`).then(res => {
                ...
                if (userSuggestId) {
                    ...
                    if (userSuggestId.classList.contains('btn-secondary'))
                        userSuggestId.classList.remove('btn-secondary')
                ...
                if (postUserId) {
                    ...
                    if (postUserId.classList.contains('btn-secondary'))
                        postUserId.classList.remove('btn-secondary')
